...
from libs.search_conditions import INCOMING_USER_INTERACT_CRITERIA
from libs.util import SegmentUtils, Utils


class IncomingCall:
    """
    Genesysユーザーへの着信に該当するsegmentを抽出してincoming webhookに送信する関数
    通話中の転送、割り込み等のないシンプルな着信を想定。着信時のフローなども設定無し

    Function to extract segments corresponding to incoming calls to Genesys users and send them to the incoming webhook
    Assumes simple incoming calls without transfers or barge-in during the call. No flow settings for incoming calls
    """
    ...
    def create_call(genesys_conversation_data, bucket_name, genesys_audio_meta_data):
        ...
        # Genesysの通話履歴データから着信に該当するsegmentを抽出。ユースケースに沿って複数segment取得はしない / Extract segments corresponding to incoming calls from Genesys call history data. Multiple segments are not retrieved depending on the use case.
        segment = SegmentUtils.get_segment(
            flatten_segments, INCOMING_USER_INTERACT_CRITERIA
        )

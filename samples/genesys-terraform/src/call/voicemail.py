...
from libs.search_conditions import INCOMING_VOICEMAIL_CRITERIA


class Voicemail:
    """
    Genesysユーザーへの留守電に該当するsegmentを抽出してincoming webhookに送信する関数
    通話中の転送、割り込み等のないシンプルな発信を想定。着信時のフローなども設定無し

    Function to extract segments corresponding to voicemails to Genesys users and send them to the incoming webhook
    Assumes simple outgoing calls without transfers or barge-in during the call. No flow settings for incoming calls.
    """
    ...
    def create_call(genesys_conversation_data, bucket_name, genesys_audio_meta_data):
        ...
        # Genesysの通話履歴データから留守電に該当するsegmentを抽出。ユースケースに沿って複数segment取得はしない / Extract segments corresponding to voicemails from Genesys call history data. Multiple segments are not retrieved depending on the use case.
        segment = SegmentUtils.get_segment(
            flatten_segments, INCOMING_VOICEMAIL_CRITERIA
        )
